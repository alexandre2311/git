...
/* global Game, GPWindowMgr, ngettext, Timestamp, DebugTranslations, GameData */
...
jQuery.blocker = function(options){
	...
	'</div></div><div class="gpwindow_top"><div class="gpwindow_left corner"></div><div class="gpwindow_right corner"></div></div>' +
...
function readableDate(date, utc, ext_date, with_seconds) {
	return readableUnixTimestamp(date.getTime(), (utc === true ? 'no_offset' : 'player_timezone'), {extended_date: ext_date, with_seconds: with_seconds});
}
...
function recalcCapacity(units, researches, pop, cap, progbar, slow_boats_needed, fast_boats_needed) {
	...
	progbar.stop(true,true).animate({backgroundPosition: progress + 'px'});
...
function isSomeThing(checks, anyOrAll) {
	...
	return function() {
		var check_idx, checks_length = checks.length, check;
...
var isiOs = isSomeThing(['iphone', 'ipad', 'ipod'], 'any');
	...
	window.getIEComputedStyle = function(el, pseudo) {
		...
		this.cssText = function() {
		    ...
		    var elStyle = el.currentStyle,
				cssText = '';
			for (var k in elStyle) { cssText += k + ':' + elStyle[k] + ';';}
			return cssText.replace(/([A-Z])/g, function($1){return '-'+$1.toLowerCase();});
		}();
